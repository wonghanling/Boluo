...
export function ProductCards() {
  ...
  if (!products.length) return <div className="h-48 animate-pulse rounded-3xl bg-white/5" />
  ...
  return (
    <div className="grid gap-5 md:grid-cols-2">
      {products.map((product) => {
        ...
        return (
          <article key={product.code} className="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
            <div className="flex items-center justify-between gap-4">
              ...
              <span className={`rounded-full px-3 py-1 text-xs ${paused ? "bg-white/10 text-white/60" : "bg-green-400/15 text-green-300"}`}>
                {paused ? "暂未开放" : "可购买"}
              </span>
            </div>
            <h2 className="mt-5 text-2xl font-bold">{product.name}</h2>
            <p className="mt-3 min-h-12 text-sm leading-6 text-white/60">{product.description}</p>
            <div className="mt-6 flex items-end justify-between gap-4">
              <div>
                <p className="text-xs text-white/40">管理员实时定价</p>
                <p className="text-3xl font-bold text-yellow-400">¥{Number(product.sale_price).toFixed(2)}</p>
              </div>
